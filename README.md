...
- The **Processor Service** uses **FFmpeg** - The **Processor Service** uses **FFmpeg** to transcode uploaded videos into multiple resolutions (`1080p`, `720p`, `360p`) and then **segment them into HLS format** (`.m3u8` playlists and `.ts` segments) for adaptive delivery. It also extracts video duration.
